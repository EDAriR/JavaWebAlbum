...
<jsp:useBean id="listPhotos_Bybaby_no" scope="request" type="java.util.List" />
...
<table border='1' bordercolor='#CCCCFF' width='800'>
	...
	<c:forEach var="photoVO" items="${listEmps_ByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr align='center' valign='middle' ${(photoVO.empno==param.empno) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色而已-->
			<td>${PhotoVO.pho_title}</td>
            <td>${PhotoVO.baby_no}</td>
			<td>${photoVO.job}</td>
			<td>${photoVO.hiredate}</td>
			<td>${photoVO.sal}</td>
			<td>${photoVO.comm}</td>			
			...
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/emp/emp.do">
			    ...
			    <input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller-->
			    <input type="hidden" name="whichPage"	value="<%=whichPage%>">               <!--送出當前是第幾頁給Controller-->
			    <input type="hidden" name="action"value="utility.delete"></FORM>
			</td>
		</tr>
	</c:forEach>
</table>
